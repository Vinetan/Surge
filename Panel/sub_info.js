...
----------------------------------------
[Script]
Sub_info = type=generic,timeout=10,script-path= https://raw.githubusercontent.com/Vinetan/Surge/main/Panel/sub_info.js,script-update-interval=0,argument=url=[URL encode 后的机场节点链接]&reset_day=1&title=AmyInfo&icon=bonjour&color=#007aff
...
(async () => {
    ...
    let content = [`𝗨𝘀𝗲𝗱 : ${toPercent(proportion)} 𝗹 𝗔𝘃𝗹 : ${bytesToSize(total-used)}`];
  ...
  */
    ...
    $done({
      title: `${args.title} ` + `𝗹 𝗥𝗲𝘀𝗲𝘁 : ` + `${resetDayLeft} 𝗗𝗮𝘆𝘀`,
      content: content.join("\n"),
      icon: args.icon || "airplane.circle",
      "icon-color": args.color || "#3378f6",
    });
  })();
